...
public class Module9_2New extends BaseClass
{
	
	SendTestReportInEmail email = new SendTestReportInEmail();
	
	
	public Module9_2New() throws Exception
	{
		super();
	...
	public void negativeScenariosTest(String testcaseName, String keyword, String objectName, String objectType,
			String value)
	{
	...
	public Object[][] getDataFromDataprovider()
	{
		...
		try
		{
			sht = file.readExcel("F:\\JAVA\\Selenium\\PallaviAssignment","Pallavi_Assignment_Validation_Object_Repository.xls", "Module9_2");
		} catch (Exception e)
		{
			// TODO Auto-generated catch block
			e.printStackTrace();
...
//			for (int j = 0; j < row.getLastCellNum(); j++)
			for (int j = 0; j < 5; j++)
			{
				...
				/*
				 * // Only need one of these DataFormatter fmt = new DataFormatter();
				 * 
				 * // Once per cell String valueAsSeenInExcel =
				 * fmt.formatCellValue(row.getCell(j));
	...
	public void tearDown(ITestResult result) throws Exception
	{
		...
		else if (result.getStatus() == ITestResult.FAILURE)
		{
			...
			test.fail(result.getThrowable().getMessage(),
					MediaEntityBuilder.createScreenCaptureFromPath(image).build());
